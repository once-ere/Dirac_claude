...
Three effects of the test set-up, not of the guide, should be known. Each replay used a home folder of its own, so that the folder `src` of Section 4.1 lay inside the test area; in PowerShell this was done by pointing the variable `USERPROFILE` elsewhere, and there the Wolfram front end then drew text slightly differently: four of the eight PNG figures of Section 11 differed from the committed ones in a few dozen pixels. The Git Bash replay, and a repetition in a fresh clone with the normal user profile, reproduced all eight byte for byte. The Git Bash and Linux replays cloned a commit in which this guide's Markdown had already been edited but its PDF not yet registered, so there the guide's own PDF build of Section 12 ended with its registry checks false; in the PowerShell replay, at a registered commit, all 14 checks passed. Finally, the computer was running other long computations during the replays, which made some runs up to five times slower than the times quoted in this guide; those were measured on the otherwise idle computer.
...
Optional: `python -m pip install jupyterlab`. If pip cannot install one of the pinned versions for your Python (the pins were installed and tested with Python 3.14.5 on Windows and Python 3.12.3 on Ubuntu 24.04), install the packages without the `==` versions, for example `python -m pip install numpy matplotlib sympy nbformat nbclient ipykernel`. Every check of this guide still passes then, but the last digits of the EXP-3 analysis files and of the checker reports can differ from the committed ones, and with a different matplotlib build the figure bytes can differ too (Sections 9.2, 10.2 and 14). The activation lasts until you close the terminal: **in every new terminal run** `source ~/.venvs/dirac16/bin/activate` again. While it is active, `python` means the Python of the environment, which is why the rest of this guide writes `python` on every platform.
...
It takes a few seconds. Running it again prints `solver_setup=ALREADY-PRESENT`. If the folder holds a different engine, the script stops with `vendor/rustSolveIt is at ..., expected ...; remove it and rerun`. If an earlier run was interrupted (for example by a network failure during the download), the folder is an incomplete checkout, and the script stops with a one-line message that says so and asks you to remove the folder and rerun (the exact text is in Section 14). In both cases delete the folder as shown in Section 4.4 and run the script again. (The PowerShell script prints these messages as a red error block and writes the folder as `vendor\rustSolveIt`.)
