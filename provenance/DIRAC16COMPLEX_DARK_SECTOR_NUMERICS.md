...
- **Does anything behave like dark matter?** Yes in its equation of state: a Pauli-consistent gas of quanta goes from $w=1/3$ to $w\to0$, pairs are created gravitationally for $m>0$, and the single-mode mean-field condensate is dust (up to a Fermi-degeneracy pressure that was not computed).
...
- **Does it fit Unite?** No: $w_0=-0.861$ forces $w_a=-4.81$, and the mean field leaves its valid regime (positive-energy occupied level) at $z=0.026$; beyond it the equations give a phantom epoch, negative energy density beyond $z=0.293$ and a bounce at $z=0.388$, which are artefacts of that approximation.
- **Deflating extra times?** Their tangent of $p/\rho$ equals the Unite fit values, but the fitted CPL parameters of that $p/\rho$ are $(-0.640,-1.98)$, and they break 4D energy conservation, the constancy of $G_N$ and the 8D energy bound.
...
5. The Unite numbers and the scalar-field formulas are taken from the reference PDF (Section 3.3), an e-mail that cites no primary publication; they were not checked against one and are not re-derived from data here.
...
**Expectation-value rule.** In the positive-norm quantization a one-particle state built on a normalised positive-energy mode $u$ above the Dirac sea has $\langle\Psi^\dagger M\Psi\rangle=u^\dagger BMu$, normal ordered relative to the sea. Hence the scalar density is $s(u)=u^\dagger BCu=u^\dagger(-i\gamma^4)u$, equal to $+1$ on every positive-energy rest eigenvector. The c-number reading $u^\dagger Cu$ is a different quantity: on the positive-energy rest eigenspace $C=B$ exactly, so $u^\dagger Cu$ is the Krein sign $\pm1$ of a joint eigenvector. The two readings agree for $B=+1$ (the states of EXP-2 and EXP-3) and differ in sign for $B=-1$ (for example the EXP-1 runs pos_Bm). The mean-field condensate is $S=S_0(V_0/V)\,s(u)$.

**Limits of the mean field.** Two limits matter for the conclusions.
...
- **Repulsive interaction.** The $\lambda=0.5$ run (pointwise, fixed-$x_0$ mean field) has $\rho=1.3318$, $\bar p=0.2471$, $w=0.1856$, $\mathrm{KE}_L=0.7780$ and $\mathrm{PE}_L=0.5538$; the six pressures $p_1,\dots,p_7$ equal $\tfrac\lambda2S^2=0.2242$. $M_{\mathrm{eff}}$ stays at $M_\ast$ to $1.3\times10^{-9}$.
...
![EXP-3 Lagrangian split: $\mathrm{KE}_L$ (solid) turns negative, and $w$ drops below $-1$, for $a<(2\lvert x_0\rvert)^{1/3}$, where the mean-field mode is a negative-energy level (Section 4.6); $\mathrm{PE}_L$ (dashed) stays positive.](artifacts/dirac16complex/numerics/figures/exp3_ke_pe.png)
...
- **Relativistic to dust.** $w=0.3328543348$ at $a=1$ (kinetic theory: the same to ten digits) and $w=0.0359224$ at $a=100$ (kinetic theory 0.0359223), both on the truncated grid $k\le k_{\max}=12T_i$. $\rho a^4$ is constant early and $\rho a^3$ late. The momentum energy $\mathrm{KE}_H=3p$ gives way to rest-mass energy.
...
- Spin independence and particle-antiparticle symmetry are exact (deviation 0.0).
...
![EXP-4 (a) the produced comoving number density $na^3$ (adiabatic basis solid, instantaneous basis dashed), and (b) the equation of state of the produced gas with its final adiabatic-basis spectrum, from nearly $1/3$ when evaluated at $a=1$ to dust.](artifacts/dirac16complex/numerics/figures/exp4_pair_density.png)
...
- **When the quanta are made.** For $m\ge0.5H_{\mathrm{inf}}$ most of the final number is produced after the de Sitter to radiation transition, in the radiation era when $H$ is of order $m$: the adiabatic-basis $na^3$ at $a=1$ in pair_history.csv is about a third to a half of its final value for $m=0.5$, 1 and 2 (for $m=0.1$ it is still evolving at $a=1$).
- **The produced gas becomes dust**: the final spectrum has $w$ between 0.24 and 0.31 when evaluated at $a=1$ and about $10^{-4}$ at the end, and $\rho a^3$ approaches $m\,na^3$ (for $m=1$: $4.413\times10^{-3}$ against $na^3=4.412\times10^{-3}$).
...
- **Free condensate, any background (single-mode mean field):** $\rho=mS\propto1/V$ and $p=0$: dust, whatever the geometry does, up to the Fermi-degeneracy pressure of a Pauli-consistent state, which was not computed (Section 4.6).
- **Repulsive condensate ($\lambda>0$):** $\rho=mS+\tfrac\lambda2S^2$ and $p=\tfrac\lambda2S^2>0$; $w\to1$ (stiff) at small $V$ and $w\to0$ at large $V$ (EXP-1 in the pointwise mean field, EXP-2).
- **Attractive condensate ($\lambda<0$):** $p<0$ and $w\to0$ at large $V$; where $M_{\mathrm{eff}}<0$ the equations give phantom $w$ and, at still smaller $V$, $\rho<0$, but there the occupied mode is a negative-energy level and these are mean-field artefacts (EXP-2, EXP-3).
- **Primordial field:** $V$ is constant, so $\rho$ and $p$ are frozen; dust or slightly stiff for the positive-energy eigenstates (EXP-1). The $2E$ oscillation of the mixed control runs is a first-quantised interference effect, not a property of the field.
- **Thermal gas:** $\rho\propto a^{-4}$ early and $\propto a^{-3}$ late; $w$ falls from 0.3329 to 0.0359 (truncated grid; 0.0361 without the momentum cut) as the momenta redshift (EXP-4).
- **Created pairs:** the final spectrum has $w$ between 0.24 and 0.31 when evaluated at $a=1$ and about $10^{-4}$ at the end of the run, and $\rho a^3\to m\,na^3$ (EXP-4); most quanta are produced in the radiation era, when $H$ is of order $m$.
...
6. **The hidden-space momentum** $k_0$ was set to zero in EXP-4 (a compact hidden dimension with a large Kaluza-Klein gap, or by hand); with it the relativistic pressure would be $\rho/4$.
...
1. **Attractive four-fermion interaction ($\lambda<0$).** The only mechanism with genuinely negative pressure, $p=\tfrac\lambda2S^2<0$. $w=x_0\sigma/(1+x_0\sigma)$ lies in $(-1,0)$ at low density. At higher density $M_{\mathrm{eff}}$ passes through zero, and beyond that point the equations give $w<-1$ with $\rho>0$ ($\mathrm{KE}_L=\tfrac12SM_{\mathrm{eff}}<0$), then $\rho<0$, and in EXP-3 a bounce (EXP-2: phantom for $0.4<V/V_0<0.8$ and $\rho<0$ below 0.4). **This phantom crossing is a mean-field artefact, not an established property of the field:** in all those rows the single occupied mode is a negative-energy eigenvector of the instantaneous $h$ ($s(u)=1$ while $M_{\mathrm{eff}}<0$), outside the domain of the expectation-value rule, and no state of positive-energy quanta has $\mathrm{KE}_L<0$ (Section 4.6).
...
7. **The deflation variant's own history.** Its $\rho_\psi$ is negative beyond $z=6.83$, and its $p/\rho$ crosses $w=-1$ at $z=0.230$; the CPL fit of that $p/\rho$ over the data range is $(-0.640,-1.98)$, not the Unite values.
8. **Krein structure and the extra-time instability.** The canonical state space is indefinite; only the good sector has a positive Fock space, and modes with extra-time momentum grow super-exponentially (EXP-5). The restriction is imposed by hand.
9. **Homogeneous mean field only.** One c-number mode stands for the condensate, at Hartree level and without the Pauli constraint on its occupation; quantum corrections, exchange terms, the stability of the condensate and inhomogeneities were not computed.
10. **No perturbations and no data likelihood.** Nothing here is a fit to supernova, BAO or CMB data; the distance comparisons use noise-free curves with equal weights, and $\Omega_m$ is fixed at the assumed 0.305 except in the $\Omega_m$-free projections of Section 12.4.
