# dirac16complex in the primordial pair-creation gravitational field

## Explicit components of the connection, field equations, energy-momentum tensor, quantization and equations of state
...
- Coordinates $x^\mu=(x_0,\dots,x_7)$, counted from 0. The roles follow the notebook: $x_0$ is the hidden space, $x_1,x_2,x_3$ are ordinary 3-space, $x_4$ is the evolution time, and $x_5,x_6,x_7$ are the three extra times (the notebook calls them superluminal deflating times).
- $z=6Hx_0\in(0,\pi/2)$, $t=Hx_4$, $s=\sin z$. $H>0$ is the notebook's single inverse length. $a_4=a_4(t)$ is an arbitrary smooth real function and $a_4'=da_4/dt$.
- $\partial_\mu=\partial/\partial x^\mu$, so $\partial_0=6H\partial_z$ and $\partial_4=H\partial_t$.
- Index groups: $i\in\{1,2,3\}$ (3-space) and $j\in\{5,6,7\}$ (extra times). "Transverse" means every direction $\mu\ne4$.
- Tangent metric $\eta=\mathrm{diag}(+1,+1,+1,+1,-1,-1,-1,-1)$. Frame directions 0 to 3 are space-like and 4 to 7 are time-like.
- Frame gamma matrices $\gamma^a=\begin{pmatrix}0&\bar\tau_a\\ \tau_a&0\end{pmatrix}$ ($a=0,\dots,7$) in the split-octonion (notebook) basis: real 16 by 16 integer matrices with $\{\gamma^a,\gamma^b\}=2\eta^{ab}$, symmetric for $a<4$ and antisymmetric for $a\ge4$. Every row of every $\gamma^a$ has exactly one nonzero entry, $+1$ or $-1$.
- A gamma matrix with a numeric upper index, $\gamma^0,\dots,\gamma^7$, is always the constant frame matrix. Curved gammas carry a Greek index, $\gamma^\mu=e_a{}^\mu\gamma^a$; with a numeric label they are written $\gamma^{x_\mu}$. Lowered gammas $\gamma_\mu=g_{\mu\nu}\gamma^\nu$ are always curved. In sums with an explicit coefficient, such as $\sum_\mu c_\mu\gamma^\mu\partial_\mu$ with $c_\mu=1/h_\mu$, $\gamma^\mu$ is the frame matrix with the numeric value of $\mu$.
- $C=\sigma_{16}=\gamma^0\gamma^1\gamma^2\gamma^3=\mathrm{diag}(-\sigma,\sigma)$ with $\sigma=\begin{pmatrix}0&I_4\\ I_4&0\end{pmatrix}$, so $C^T=C$ and $C^2=1$. The Dirac adjoint is $\bar\Psi=\Psi^\dagger C$.
- $\gamma^8=\gamma^0\gamma^1\cdots\gamma^7=\mathrm{diag}(-I_8,I_8)$: $\Psi_0,\dots,\Psi_7$ have chirality $-1$ and $\Psi_8,\dots,\Psi_{15}$ have chirality $+1$.
...
1. **Inflation of 3-space.** The three space directions have the scale factor $s^{1/6}e^{a_4(t)}$ and the rate $\partial_4\ln(s^{1/6}e^{a_4})=Ha_4'$. For $a_4=t$ ordinary 3-space expands exponentially, as $e^{Hx_4}$, at the rate $H$. This is what the notebook calls superluminal inflation.
2. **Deflation of the three extra times.** The directions $x_5,x_6,x_7$ have the scale factor $s^{1/6}e^{-a_4(t)}$ and the rate $-Ha_4'$.
3. **Constant 7-volume.** The six transverse scale factors multiply to $s$, independent of $a_4$. Hence $\sqrt{|g|}=\cos z$ does not depend on $x_4$: the inflation of 3-space is exactly compensated by the deflation of the extra times.
4. **Hidden-space warp.** In the coordinate $\zeta$ of Section 4.4 the six transverse directions carry the common factor $e^{2H\zeta}$ over the hidden coordinate $\zeta\in(-\infty,0)$, an exponentially warped product.
5. **Control.** One scale $H$ and one free function $a_4$. The metric is diagonal and depends on $x_0$ and $x_4$ only. Its spin connection enters the Dirac operator only through $\gamma^\mu\Omega_\mu=3H\gamma^0$, a constant multiple of $\gamma^0$, and its Einstein tensor is diagonal and independent of $x_0$.
...
### 4.2 The z, t form and the vielbein
...
### 4.3 Determinant, volume and signature
...
### 7.2 Explicit action on the components
...
### 7.3 Curvature and the Lichnerowicz identity
...
### 8.1 The contraction with the curved gammas
...
### 8.2 Covariant constancy of the gammas
...
### 9.3 Relation to the notebook Lagrangian
...
### 10.2 Component form in all eight coordinates
...
### 10.3 Other presentations of the equations
...
### 10.5 The (x0, x4) block form and the z, t evolution form
...
### 11.1 Variables and cross reference
...
1. With Clifford-consistent curved gammas $\gamma^\mu=e_a{}^\mu\gamma^a$, every $Q_1$ term drops out of the commuting-field Euler-Lagrange equations, both for the notebook contraction and for the correct $\Omega_\mu$. The reason is that $\sigma_{16}\gamma^\mu\Omega_\mu$ has no symmetric part for a diagonal vielbein. For commuting fields the gravitational term of the notebook's equations comes from $\partial_\mu(\sqrt{|g|}\gamma^\mu)$, not from the spin connection.
2. The stored eLa (cell 1079) contains $Q_1$ terms in exactly the 8 rows of the coupling sets $\{0,5,8,13\}$ and $\{1,4,9,12\}$.
3. These terms are reproduced exactly, 16/16 rows with zero residual, by the reconstruction in which the curved gammas $\gamma^{x_5},\gamma^{x_6},\gamma^{x_7}$ of useT16 carry the factor $e^{-a_4}s^{-1/6}$ on their $+1$ entries and $e^{+a_4}s^{-1/6}$ on their $-1$ entries:
...
**The rule.** Cell 1058 contains the substitution rule
...
### 12.1 Definition and form in this field
...
- **$K=0$, isotropic.** $\rho=mS+U$ and $p=SU'-U$ in all seven transverse directions, so the equation of state is $w=\dfrac{SU'-U}{mS+U}=\dfrac{\mathrm{KE}_L-\mathrm{PE}_L}{\mathrm{KE}_L+\mathrm{PE}_L}$.
- **Free massive ($U=0$, $K=0$).** $\mathrm{KE}_L=\mathrm{PE}_L=\tfrac12mS$ and $p=0$: $w=0$, dust.
- **Default potential ($U=\tfrac\lambda2S^2$, $K=0$).** $\rho=mS+\tfrac\lambda2S^2$, $p=\tfrac\lambda2S^2$, $w=\dfrac{\lambda S}{2m+\lambda S}$, $\mathrm{KE}_L=\tfrac12S(m+\lambda S)$ and $\mathrm{PE}_L=\tfrac12mS$. For $\rho>0$ the state is phantom ($w<-1$) exactly when $\mathrm{KE}_L<0$, that is when $S(m+U')<0$.
...
### 15.2 Source required by 8D Einstein gravity
...
### 15.4 Linear a4 and the cell-150 alternatives
...
- 13 of the 28 commute with $B$: the 9 with $a,b$ both in $\{0,1,2,3\}$ or both in $\{5,6,7\}$, plus the 4 boosts $S^{a4}$ with $a\in\{0,1,2,3\}$.
- The 9 compact ones are anti-Hermitian and generate the unitarily implemented $\mathrm{Spin}(4)\times\mathrm{Spin}(3)$. The 4 boosts commute with $B$ but are Hermitian, not unitary.
- The 21 generators with $a,b\ne4$ are Krein-unitary (Spin(4,3)); the 7 generators $S^{4b}$ fail the Krein condition.
...
### 17.3 Hamiltonian density and Heisenberg equations
...
Its last line is `stage2_primordial_field_verification=OK`.
