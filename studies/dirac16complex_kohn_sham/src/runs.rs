...
pub fn config_lines() -> Vec<String> {
    vec![
        ...
        "occupations      = Fermi-Dirac, normal ordering (sea holes = antiparticles), mu by bisection; window tail f_cut 1e-8".to_string(),
        "T = 0 fallback   = exact aufbau; a stagnant level crossing is re-solved with occupation smearing 1e-4 m, 1e-3 m (recorded)".to_string(),
        "couplings        = per (m, L, N): lambda_hat_1 = 0.1/strength, lambda_hat_2 = 1.0/strength, strength = max_y max((15/16)|S_p|, n_p/16)/m^7 of the free ground state".to_string(),
