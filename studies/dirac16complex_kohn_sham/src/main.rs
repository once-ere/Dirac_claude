...
//! line is `SUCCESS` (exit code 0) or `FAILURE` (exit code 1).
...
const SUBCOMMANDS: [(&str, Runner); 5] = [
    ("spectrum", runs::run_spectrum),
    ("scf", runs::run_scf),
    ("excited", runs::run_excited),
    ("thermo", runs::run_thermo),
    ("emt", runs::run_emt),
];
...
fn print_config(ctx: &RunContext) -> bool {
    ...
    println!("quick            = {}", ctx.quick);
    for line in runs::config_lines() {
        println!("{line}");
    }
...
fn main() {
    ...
    let passed = match parse(&arguments) {
        ...
        Ok((command, ctx)) => match command.as_str() {
            ...
            "all" => {
                let mut all = true;
                for (name, runner) in SUBCOMMANDS {
