// Origin: studies/dirac16complex_cosmology/src/driver.rs (Stage 3 of this
// repository, GPL-3.0-or-later), copied unchanged into the Stage-4 crate on
// 2026-09-25 so that both crates share one verified CVODE driver.
...
//!   objects exist.
//! * Stage-4 addition (`integrate_adjusted`): after every reached target an
//!   `adjust` closure may modify the state in place (the shooting module
//!   rescales exponentially growing linear solutions); the driver then calls
//!   `CVodeReInit` at that time with the modified state, which restarts the
//!   multistep history exactly as SUNDIALS prescribes for a discontinuous
//!   state change.  `integrate` is `integrate_adjusted` with no adjustment.
...
/// State adjustment hook: returns true when the state was modified.
pub type AdjustFn<'a> = &'a mut dyn FnMut(f64, &mut [f64]) -> bool;
...
fn march(
    ...
    for &target in targets {
        ...
        result.times.push(t);
        let mut state = read_vector(y, n)?;
        if let Some(hook) = adjust.as_mut() {
            if hook(t, &mut state) {
                {
                    let mut data = N_VGetArrayPointer(y)
                        .ok_or_else(|| "N_VGetArrayPointer returned None for y".to_string())?;
                    data[..n].copy_from_slice(&state);
                }
                flag_check(CVodeReInit(cvode, t, y), "CVodeReInit")?;
            }
        }
...
/// stored state is the adjusted one; CVODE is re-initialised there).
pub fn integrate_adjusted(
    y0: Vec<f64>,
    t0: f64,
    targets: &[f64],
    rhs: RhsFn,
...
mod tests {
    ...
    #[test]
    fn adjusted_integration_rescales_and_reinitialises() {
        // y' = y, rescale to 1 at every target: the stored states are all 1,
        // and the product of the scale factors is e^{t}.
        let rhs: RhsFn = Box::new(|_t, y, ydot| {
            ydot[0] = y[0];
            Ok(())
        });
        let cfg = SolverConfig::bdf(1e-10, 1e-12, 0.01);
        let targets = uniform_targets(0.0, 3.0, 6);
        let mut log_scale = 0.0;
        let mut adjust = |_t: f64, state: &mut [f64]| {
            log_scale += crate::math::log(state[0]);
            state[0] = 1.0;
            true
        };
        let run = integrate_adjusted(vec![1.0], 0.0, &targets, rhs, &cfg, &mut adjust).unwrap();
        for y in &run.states {
            assert_eq!(y[0], 1.0);
        }
